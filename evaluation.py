...
if __name__ == "__main__":
    ...
    print('**************************************************')
    print(('&{:.4f}' *6).format(np.average(total_dict_merge["abs_rel"]), np.average(total_dict_merge["a1"]), np.var([np.average(total_dict[env]["abs_rel"]) for env in total_dict.keys()]) * 100, np.var([np.average(total_dict[env]["a1"]) for env in total_dict.keys()]) * 100, rng([np.average(total_dict[env]["abs_rel"]) for env in total_dict.keys()]), rng_a1([np.average(total_dict[env]["a1"]) for env in total_dict.keys()])))
